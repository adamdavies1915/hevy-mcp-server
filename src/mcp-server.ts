...
export const SERVER_NAME = "Hevy API";
export const SERVER_VERSION = "3.1.0";

/**
 * Builds an MCP server instance bound to one authenticated user.
 *
 * On Workers this was a Durable Object (McpAgent) whose lifetime spanned a
 * session; under Node a fresh instance is created per MCP session by
 * routes/mcp.ts and the Hevy client is captured in the closure below.
 */
export async function createHevyMcpServer(
	...
	if (!props || !props.login) {
		const setupHint = props?.baseUrl
			? ` Visit ${props.baseUrl}/setup to get started.`
	...
	if (!isAllowedUser(env, props.login)) {
		throw new Error(`GitHub user ${props.login} is not permitted to use this server.`);
	}

	// Prefer the key this user stored via /setup; fall back to the deployment's
	// own HEVY_API_KEY for single-user installs that configure it in the
	// environment instead.
	const hevyApiKey =
		(await getUserApiKey(env.OAUTH_KV, env.COOKIE_ENCRYPTION_KEY, props.login)) ??
		env.HEVY_API_KEY ??
		null;

	if (!hevyApiKey) {
		const setupUrl = props.baseUrl
			? `${props.baseUrl}/setup`
			: '/setup (visit your server URL)';
		throw new Error(
			`Hevy API key not configured for user ${props.login}. ` +
